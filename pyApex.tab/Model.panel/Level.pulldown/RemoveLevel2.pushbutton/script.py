...
def level_dependent(level):
    """By level"""

    selected_levels = [level]

    views_to_close = check_dependent_views(level)
    ...
    t = Transaction(doc, "Check level " + level.Name)
    t.Start()
    elements = doc.Delete(level.Id)
    t.RollBack()
...
def main():
    ...
    for l in selected_levels1:
        elements = level_dependent(l)

        targetTrans = Transaction(doc)
        targetTrans.Start("CopyBetweedDocs")
        copiedId = []
        errors = 0      
        try:
            doc.Delete(l.Id)
            copiedId = ElementTransformUtils.CopyElements(doc, elements, XYZ(0,0,0))
        except Exception as e:
